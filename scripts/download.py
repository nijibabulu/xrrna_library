...
def main(accession_list, output_dir, molecule, format, api_key):
    config = ncbi_acc_download.core.Config(
        molecule=molecule, format=format, api_key=api_key, keep_filename=True
    )
    with open(accession_list) as f, open(output_dir / "err.txt", "w") as log:
        for line in f:
            ...
            try:
                ...
                try:
                    ncbi_acc_download.core.download_to_file(accession + ".1", config, out_path)
                except InvalidIdError as _:
